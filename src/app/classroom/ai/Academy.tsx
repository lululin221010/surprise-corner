...
const UNLOCK_KEYS: Record<string, string> = {
  'ai-intro':    'ss-ai-intro',
  'ai-advanced': 'ss-ai-advanced',
  'ai-master':   'ss-ai-master',
};
...
export default function Academy() {
  ...
  // 課程列表（選了某個課程）
  if (activeCourse) {
    const isTrial = activeCourse.id === 'ai-trial';
    ...
    return (
      <div className="classroom-content">
        ...
        <div className="classroom-breadcrumb" style={{ marginBottom: '1rem' }}>
          ...
          <button onClick={() => setActiveCourse(null)} className="classroom-back" style={{ fontSize: '12px' }}>AI書院</button>
          <span style={{ margin: '0 0.4rem' }}>›</span>
        ...
        {!isTrial && !isUnlocked && (
          <div style={{ background: 'rgba(251,191,36,0.06)', border: '1px solid rgba(251,191,36,0.3)', borderRadius: '12px', padding: '1rem', marginBottom: '1.2rem' }}>
            ...
            <a
              href={PURCHASE_URLS[activeCourse.id] ?? 'https://still-time-corner.vercel.app/ebooks'}
              target="_blank" rel="noopener noreferrer"
              style={{ display: 'block', background: 'linear-gradient(135deg, #7c3aed, #2563eb)', color: '#fff', fontWeight: 700, fontSize: '0.82rem', borderRadius: '20px', padding: '0.5rem', textDecoration: 'none', textAlign: 'center' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column' }}>
            {activeCourse.lessons.map((lesson, i) => {
              ...
              return (
                ...
                >
                  <div style={{ width: '30px', height: '30px', borderRadius: '50%', flexShrink: 0, background: done ? '#dcfce7' : locked ? '#f3f4f6' : '#ede9fe', display: 'flex', alignItems: 'center', justifyContent: 'center', color: done ? '#15803d' : locked ? '#9ca3af' : '#7c3aed', fontSize: '0.78rem', fontWeight: 700 }}>
                    {done ? '✓' : locked ? '🔒' : i + 1}
                  </div>
                  <div style={{ flex: 1 }}>
    ...
    <div className="classroom-content">
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
        {allCourses.map(course => {
          const isTrial = course.id === 'ai-trial';
          ...
          return (
            <button
              ...
              style={{ padding: '1.2rem 1.4rem', borderRadius: '14px', border: `1px solid ${isUnlocked && !isTrial ? 'rgba(34,197,94,0.3)' : 'rgba(196,181,253,0.4)'}` }}
            >
              ...
              <div style={{ flex: 1, textAlign: 'left' }}>
                ...
                <div style={{ color: '#a78bfa', fontSize: '0.72rem', marginTop: '0.4rem' }}>
                  {course.lessons.length} 堂課{course.lessons.length === 0 ? '（建置中）' : ''}
                </div>
              </div>
