'use client';
// 📄 路徑：src/app/classroom/ai-diy/Academy.tsx
...
const UNLOCK_KEYS: Record<string, string> = {
  'diy-intro':    'ss-diy-intro',
  'diy-advanced': 'ss-diy-advanced',
  'diy-master':   'ss-diy-master',
};

const PURCHASE_URLS: Record<string, string> = {
  'diy-intro':    'https://still-time-corner.vercel.app/digital/6a4e093526746b18ddee8049',
  'diy-advanced': 'https://still-time-corner.vercel.app/digital/6a4e093526746b18ddee804a',
  'diy-master':   'https://still-time-corner.vercel.app/digital/6a4e093526746b18ddee804b',
};
...
export default function Academy() {
  ...
  const [unlockedCourses, setUnlockedCourses] = useState<Set<string>>(() => {
    ...
    const keys = ['ss-diy-intro', 'ss-diy-advanced', 'ss-diy-master'];
    return new Set(keys.filter(k => localStorage.getItem(`sc_ai_unlock_${k}`) === 'true'));
  });

  const effectiveUnlocked = isPreview
    ? new Set(['ss-diy-intro', 'ss-diy-advanced', 'ss-diy-master'])
    : unlockedCourses;
    ...
    return (
      <div className="classroom-content">
        ...
        <div className="classroom-breadcrumb" style={{ marginBottom: '1rem' }}>
          ...
          <button onClick={() => setActiveCourse(null)} className="classroom-back" style={{ fontSize: '12px' }}>DIY AI實作書院</button>
          <span style={{ margin: '0 0.4rem' }}>›</span>
