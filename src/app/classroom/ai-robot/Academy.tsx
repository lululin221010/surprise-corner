'use client';
// 📄 路徑：src/app/classroom/ai-robot/Academy.tsx
...
const UNLOCK_KEYS: Record<string, string> = {
  'robot-intro':    'ss-robot-intro',
  'robot-advanced': 'ss-robot-advanced',
  'robot-master':   'ss-robot-master',
};

const PURCHASE_URLS: Record<string, string> = {
  'robot-intro':    'https://still-time-corner.vercel.app/digital/6a4e0efaf6ba31ff23ec0a97',
  'robot-advanced': 'https://still-time-corner.vercel.app/digital/6a4e0efaf6ba31ff23ec0a98',
  'robot-master':   'https://still-time-corner.vercel.app/digital/6a4e0efaf6ba31ff23ec0a99',
};

const PRICES: Record<string, string> = {
  'robot-intro':    'NT$249',
  'robot-advanced': 'NT$249',
  'robot-master':   'NT$249',
};
...
export default function Academy() {
  ...
  const [unlockedCourses, setUnlockedCourses] = useState<Set<string>>(() => {
    ...
    const keys = ['ss-robot-intro', 'ss-robot-advanced', 'ss-robot-master'];
    return new Set(keys.filter(k => localStorage.getItem(`sc_ai_unlock_${k}`) === 'true'));
  });

  const effectiveUnlocked = isPreview
    ? new Set(['ss-robot-intro', 'ss-robot-advanced', 'ss-robot-master'])
    : unlockedCourses;
    ...
    return (
      <div className="classroom-content">
        ...
        <div className="classroom-breadcrumb" style={{ marginBottom: '1rem' }}>
          ...
          <button onClick={() => setActiveCourse(null)} className="classroom-back" style={{ fontSize: '12px' }}>機器人書院</button>
          <span style={{ margin: '0 0.4rem' }}>›</span>
