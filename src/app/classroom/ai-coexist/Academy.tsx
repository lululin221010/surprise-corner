'use client';
// 📄 路徑：src/app/classroom/ai-coexist/Academy.tsx
...
const UNLOCK_KEYS: Record<string, string> = {
  'coexist-intro':    'ss-coexist-intro',
  'coexist-advanced': 'ss-coexist-advanced',
  'coexist-master':   'ss-coexist-master',
};

const PURCHASE_URLS: Record<string, string> = {
  'coexist-intro':    'https://still-time-corner.vercel.app/digital/6a4e04ead2cf6221cbf426d1',
  'coexist-advanced': 'https://still-time-corner.vercel.app/digital/6a4e04ead2cf6221cbf426d2',
  'coexist-master':   'https://still-time-corner.vercel.app/digital/6a4e04ead2cf6221cbf426d3',
};

const PRICES: Record<string, string> = {
  'coexist-intro':    'NT$249',
  'coexist-advanced': 'NT$249',
  'coexist-master':   'NT$249',
};
...
export default function Academy() {
  ...
  const [unlockedCourses, setUnlockedCourses] = useState<Set<string>>(() => {
    ...
    const keys = ['ss-coexist-intro', 'ss-coexist-advanced', 'ss-coexist-master'];
    return new Set(keys.filter(k => localStorage.getItem(`sc_ai_unlock_${k}`) === 'true'));
  });

  const effectiveUnlocked = isPreview
    ? new Set(['ss-coexist-intro', 'ss-coexist-advanced', 'ss-coexist-master'])
    : unlockedCourses;
    ...
    return (
      <div className="classroom-content">
        ...
        <div className="classroom-breadcrumb" style={{ marginBottom: '1rem' }}>
          ...
          <button onClick={() => setActiveCourse(null)} className="classroom-back" style={{ fontSize: '12px' }}>互動共存書院</button>
          <span style={{ margin: '0 0.4rem' }}>›</span>
