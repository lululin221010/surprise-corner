'use client';
// 📄 路徑：src/app/classroom/ai-thinking/Academy.tsx
...
const UNLOCK_KEYS: Record<string, string> = {
  'thinking-intro':    'ss-thinking-intro',
  'thinking-advanced': 'ss-thinking-advanced',
  'thinking-master':   'ss-thinking-master',
};

const PURCHASE_URLS: Record<string, string> = {
  'thinking-intro':    'https://still-time-corner.vercel.app/digital/6a4dfcf4888c2de7ae2caff0',
  'thinking-advanced': 'https://still-time-corner.vercel.app/digital/6a4dfcf4888c2de7ae2caff1',
  'thinking-master':   'https://still-time-corner.vercel.app/digital/6a4dfcf4888c2de7ae2caff2',
};

const PRICES: Record<string, string> = {
  'thinking-intro':    'NT$249',
  'thinking-advanced': 'NT$249',
  'thinking-master':   'NT$249',
};
...
export default function Academy() {
  ...
  const [unlockedCourses, setUnlockedCourses] = useState<Set<string>>(() => {
    ...
    const keys = ['ss-thinking-intro', 'ss-thinking-advanced', 'ss-thinking-master'];
    return new Set(keys.filter(k => localStorage.getItem(`sc_ai_unlock_${k}`) === 'true'));
  });

  const effectiveUnlocked = isPreview
    ? new Set(['ss-thinking-intro', 'ss-thinking-advanced', 'ss-thinking-master'])
    : unlockedCourses;
    ...
    return (
      <div className="classroom-content">
        ...
        <div className="classroom-breadcrumb" style={{ marginBottom: '1rem' }}>
          ...
          <button onClick={() => setActiveCourse(null)} className="classroom-back" style={{ fontSize: '12px' }}>AI思考力書院</button>
          <span style={{ margin: '0 0.4rem' }}>›</span>
