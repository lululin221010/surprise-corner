'use client';
// 📄 路徑：src/app/classroom/ai-communication/Academy.tsx
...
const UNLOCK_KEYS: Record<string, string> = {
  'prompt-intro':    'ss-prompt-intro',
  'prompt-advanced': 'ss-prompt-advanced',
  'prompt-master':   'ss-prompt-master',
};

const PURCHASE_URLS: Record<string, string> = {
  'prompt-intro':    'https://still-time-corner.vercel.app/digital/6a48dfbe3312259471fbc020',
  'prompt-advanced': 'https://still-time-corner.vercel.app/digital/6a48e6a398ea9a22363ec323',
  'prompt-master':   'https://still-time-corner.vercel.app/digital/6a48e81cca89f630636fe208',
};

const PRICES: Record<string, string> = {
  'prompt-intro':    'NT$249',
  'prompt-advanced': 'NT$249',
  'prompt-master':   'NT$249',
};
...
export default function Academy() {
  ...
  const [unlockedCourses, setUnlockedCourses] = useState<Set<string>>(() => {
    ...
    const keys = ['ss-prompt-intro', 'ss-prompt-advanced', 'ss-prompt-master'];
    return new Set(keys.filter(k => localStorage.getItem(`sc_ai_unlock_${k}`) === 'true'));
  });

  const effectiveUnlocked = isPreview
    ? new Set(['ss-prompt-intro', 'ss-prompt-advanced', 'ss-prompt-master'])
    : unlockedCourses;
  ...
  if (activeLesson && activeCourse) {
    const isFree = activeCourse.id === 'prompt-trial';
    return (
      <AcademyLesson
        lesson={activeLesson}
  ...
  if (activeCourse) {
    const isTrial = activeCourse.id === 'prompt-trial';
    ...
    return (
      <div className="classroom-content">
        ...
        <div className="classroom-breadcrumb" style={{ marginBottom: '1rem' }}>
          ...
          <button onClick={() => setActiveCourse(null)} className="classroom-back" style={{ fontSize: '12px' }}>AI溝通學書院</button>
          <span style={{ margin: '0 0.4rem' }}>›</span>
    ...
    <div className="classroom-content">
      <div style={{ marginBottom: '1.5rem' }}>
        ...
        <p style={{ color: '#6b7280', fontSize: '0.85rem', margin: 0 }}>
          3冊・32堂，從Prompt理解、Agent行動到AI記憶，搞懂怎麼跟AI「講話」跟「共事」。不需技術背景，每堂15-20分鐘。
        </p>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.8rem' }}>
        {allCourses.map(course => {
          const isTrial = course.id === 'prompt-trial';
